...
function getOverviewData($DbConn, $TableOrdering = "createdNewestFirst", $showPendingJobs = true, $showWorkInProgressJobs = true, $showCompletedJobs = true, $UseDateCreatedRange = false, $DateCreatedStart = "", $DateCreatedEnd = "", $UseDateDueRange = false, $DateDueStart = "", $DateDueEnd = "", $useDateTimeWorkedRange = false, $dateTimeWorkStart = "", $dateTimeWorkEnd = "", $excludeUnworkedJobs = false, $UseSearchKey = false, $SearchKey = "", $ShowOnlyUrgentJobs = false, $ShowOnlyNonurgentJobs = false, $subSortByPriority = false, $showSeconds = false)
{
    ...
    for($i = 0; $i < $res->num_rows; $i++)
    {
        ...
        $totalWorkedTime	= durationToTime($row['totalWorkedDuration'], $showSeconds);
        $totalOvertime 		= durationToTime($row['totalOvertimeDuration'], $showSeconds);
        $expectedTime 		= durationToTime($row['expectedDuration'], $showSeconds);
		...
		$chargePerMinStr = ""; // commented the actual line (bottom), this is a temporary fixture.
